...
/** Parecido mínimo, con las confusiones típicas del reconocedor ya descontadas, para aceptar una palabra. */
const UMBRAL_PALABRA = 0.7;
 ...
 */
export function aceptaPalabra(esperado: string, oido: string, alternativas?: string[]): boolean {
  if (!alternativas) return phonetics.similarity(oido, esperado) >= UMBRAL_PALABRA;

  const buscada = phonetics.normalize(esperado);
  const hipotesis = alternativas.slice(0, HIPOTESIS_PALABRA);
  if (hipotesis.some((h) => phonetics.normalize(h.replace(/ /g, '')) === buscada)) return true;
  ...
  if (otraPalabra && parecidoFonetico(primera, esperado) >= UMBRAL_PALABRA) return false;

  return parecidoFonetico(oido, esperado) >= UMBRAL_PALABRA;
}

/** Acepta si alguna de las mejores hipótesis tiene todas las palabras de la oración, en orden. */
export function aceptaOracion(esperado: string, candidatos: string[]): boolean {
  return candidatos.slice(0, HIPOTESIS_ORACION).some((c) => costoOracion(c, esperado) <= TOLERANCIA_ORACION);
}
 ...
 */
export function parecidoFonetico(oido: string, esperado: string): number {
  const a = phonetics.normalize(oido).replace(/ /g, '');
  const b = phonetics.normalize(esperado).replace(/ /g, '');
  if (!a || !b) return 0;

  const costo = (x: string, y: string) => {
    if (x === y) return 0;
    if (VOCALES.has(x) && VOCALES.has(y)) return 0.5;
...
function peso(palabra: string): number {
  return phonetics.normalize(palabra).length <= 2 ? 0.5 : 1;
}
 ...
 */
function costoOracion(oido: string, esperado: string): number {
  ...
  for (let i = 0; i <= E.length; i += 1) {
    for (let j = 0; j <= O.length; j += 1) {
      ...
      if (i < E.length) d[i + 1][j] = Math.min(d[i + 1][j], base + peso(E[i]));
      if (j < O.length) d[i][j + 1] = Math.min(d[i][j + 1], base + peso(O[j]));
      if (i === E.length) continue;
      for (let k = 1; k <= 3 && j + k <= O.length; k += 1) {
        const junto = O.slice(j, j + k).join('');
        const coincide = phonetics.similarity(junto, E[i]) >= UMBRAL_PALABRA_EN_ORACION;
        d[i + 1][j + k] = Math.min(d[i + 1][j + k], base + (coincide ? 0 : peso(E[i])));
      }
    }
  }
